...
### 採用予定アーキテクチャ（Cloudflare R2 案 ← Google Drive 案から変更）

Google Drive 案は「管理者が Drive とシステムを両方操作する」手動運用になり使い勝手が悪いため、**Cloudflare R2 をストレージとして使う案が現実的**という結論になった（2026-02-26）。
...
管理者  →  イベントモーダルの「ファイルを添付」をタップ
              ↓ 端末のファイル選択画面
              ↓ ファイルを選ぶだけ（自動アップロード）
           Cloudflare R2 にファイルを保存
              ↓
           URL + メタ情報を Firestore のイベントに保存

ユーザー →  イベントモーダルを開く → 添付ファイル一覧を表示
              ↓ ファイルをタップ
           R2 から直接ダウンロード（Firebase を経由しない）
```

**技術的な流れ（Cloudflare R2）:**
```
管理者がファイルを選択（<input type="file">）
    → Cloudflare Workers で署名付きURL発行
    → ブラウザから R2 に直接アップロード
    → 完了後、URL + メタ情報を Firestore に保存
    → モーダルの添付ファイル一覧に表示
```

**Cloudflare R2 の無料枠：**
- 保存データ 10 GB/月
- DL転送量 **無料**（何人がDLしても課金なし）
- アップロード 100万回/月

**Firebase Storage を使う場合（実装がよりシンプルな代替案）:**
- Cloudflare Workers 不要、Firebase SDK でそのまま実装可能
- 転送量（1GB/日）の制限だけ許容できれば最も実装コストが低い
- 小規模チーム（保護者数十人）なら実質問題になる可能性は低い
...
events/{eventId}/
  attachments: [
    {
      name: "大会要綱.pdf",
      url: "https://r2.example.com/...",   // R2 or Firebase Storage の URL
      size: "512KB",
...
### ホスティング移行の検討（2026-02-26）

現在 Firebase Hosting で運用中だが、将来的に Cloudflare Pages への移行も選択肢：
- Cloudflare Pages も GitHub と連携し、push → 自動デプロイが動く
- Auth・Firestore は Hosting に依存しないのでそのまま使い続けられる
- R2 と Pages を同じ Cloudflare に統一するとダッシュボードが一元管理できる
- **ただし現時点では Firebase Hosting で問題ないため、ファイル添付実装のタイミングで合わせて検討する**
