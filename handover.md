...
## 2026-07-24 コードベース徹底監査＋Playwright実操作によるUI改善

**ユーザー指示**: 「コードベースで徹底的にバグを探して修整して。それが完了したら
playwriteで実際にアプリを操作してuiの改善箇所等を見つけて直しておいて」

### バグ監査で見つけて修正した点
1. **`isTargetFor`ロジックの重複** — `js/hub/admin.js`の`loadAttendanceMatrix`と
   `loadAttendanceRates`に、対象学年・追加選手を判定する同一ロジックが
   2箇所に手書きで重複していた（将来の改修で片方だけ直して食い違う
   ドリフトのリスク）。`FCOjima.Utils.isTargetFor`に一本化して共通化。
2. **連絡事項検索のself-XSS** — `js/carpool/notifications.js`の
   `searchNotifications`で、検索キーワードをエスケープせず`innerHTML`に
   直接埋め込んでいた箇所が2箇所（該当なしメッセージ・検索結果ヘッダー）。
   `UI.escapeHTML()`でエスケープするよう修正。
3. **出欠確認ページの期限表示が空でも枠だけ残る** — `js/carpool/attendance.js`の
   出欠確認ページで、イベントに出欠期限（`attendanceDeadline`）が
   設定されていない場合、`#attendance-deadline`の中身を空にするだけで
   `class`（背景色・左ボーダー付き）は残したままにしていたため、
   何も書かれていない空の枠だけが画面に表示され続けていた。
   期限未設定時は`display:none`で要素ごと隠すよう修正。

### Playwrightで実際に操作して見つけたUI改善点
- メンバータブなどの3〜4個並ぶアクションボタンが、モバイル幅で
  1ボタンずつ3行に折り返され、しかも単語の途中で改行されていた
  → `.action-buttons`をflex-wrapから2列CSS Gridに変更（ボタン単体の
  場合は1行フル幅になるよう`:only-child`も対応）。
- 検索ボックスの「検索」ボタンが2行に折り返される → 
  `.search-box button, .filter-controls button`に
  `flex-shrink:0; white-space:nowrap`を追加。
- 添付ファイルの`<input type="file">`が素のブラウザ標準スタイルのままで
  アプリのデザインと不釣り合い → `::file-selector-button`にデザイン
  システムに合わせたスタイルを追加。

### 検証
- 既存回帰スイート `e2e-feedback7.mjs`（13項目）・
  `e2e-overflow-filelimit.mjs`（8項目）を再実行し全PASS、回帰なし。
- 出欠期限の修正は新規に`e2e-deadline-fix.mjs`で
  「期限未設定時に`#attendance-deadline`が`display:none`になり
  空枠が残らない」ことを確認（PASS）。
- Playwrightでhub/carpool全25画面（カレンダー・各種モーダル・管理画面・
  メンバー・会場登録・保護者ビュー・配車管理全タブ）を実際に操作して
  スクリーンショットを取得し目視確認。

### キャッシュバスト
`utils.js`/`admin.js`/`notifications.js` → `?v=20260722`、
`attendance.js` → `?v=20260724`、`common.css` → `?v=20260724`

---

## 2026-07-21 天気はみ出しの根本原因を特定・修正＋添付ファイル1イベント10個上限

**ユーザー報告（実機スクショ2枚）**: 「配車管理画面のイベント概要で天気予報が左右にはみ出す」
「一つのイベントに登録できるファイルは10個まで」

### ① 天気はみ出し — 前回の修正（`.weather-widget{max-width:100%}`）だけでは直っていなかった
**根本原因**: `css/carpool/overview.css` に、現在は一切使われていない**旧デザインの
カード用CSS**（`.event-info-card`/`.event-info`/`.event-details`(display:grid)/
`.event-detail-item`等。grepで実装側から一切参照されていないことを確認済み）が残っており、
その中の `.event-details { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); }`
が、`carpool/index.html` の `<div id="event-summary" class="event-details">` に
**クラス名の偶然の一致で意図せず適用**されていた。
CSS Gridの子要素はデフォルトで `min-width: auto`（コンテンツの内在最小幅）を持つため、
天気ウィジェットの横スクロール行のような「折り返さない幅広コンテンツ」を
イベント概要に追加した途端、グリッドトラックごと押し広げられ、ページ全体が
左右にはみ出していた。前回の`.weather-widget`単体へのmax-width指定は、
広げられた側のグリッドトラック自体には無力だった。

**修正**: 未使用と確認できた旧CSS一式を削除し、`#event-summary`を明示的に
`display: block` に固定。

**検討して撤回した案**: `html, body { overflow-x: hidden }` によるページレベルの
安全策も試したが、`overflow-x` を明示指定すると仕様上 `overflow-y` が
暗黙的に `auto` になり、**hubの上部固定ナビ（position:sticky）が機能しなくなる
重大な回帰**をPlaywrightで実測確認したため不採用。根本原因（意図しないCSS Grid化）
を直接断つ方針に切り替えた。

### ② 添付ファイルは1イベント10個まで
`js/common/eventfiles.js` の `EventFiles.upload()` に、アップロード前に
既存添付数を `listAll()` で数えて上限チェックする処理を追加（`MAX_FILES = 10`）。
上限超過分は弾き、アップロード可能な残数だけ通す。配車概要ページ独自の
`Overview.uploadEventFiles`（削除ボタン付き別実装）にも同じチェックを追加。
一覧表示にも「添付 X/10」の件数を常時表示。

### 検証
E2E `e2e-overflow-filelimit.mjs` 8項目 全PASS。実機報告の再現条件
（荷物12点・長い持ち物文字列・天気ウィジェット同時表示）で
`#event-summary`が`display:block`に修正され、ページ・ウィジェットとも
横スクロールが発生しないことを確認。**sticky回帰がないことも明示的に検証**
（hubの上部固定ナビがスクロール後もtop:0を維持）。添付10個の頭打ち・
満杯時の追加拒否・件数表示も確認。既存のluggage/feedback7スイート
（19+13項目）も再実行し回帰なし。

### キャッシュバスト
`eventfiles.js`/`overview.js`/`overview.css`/`common.css` → `?v=20260721b`

---

## 2026-07-21 荷物（共用備品）機能＋添付ファイル強化＋持ち物デフォルト変更ほか

**ユーザー要望（実機スクショ3枚＋詳細指示）と対応**:

1. **天気ウィジェットのはみ出し**: 配車概要で左右にはみ出していた →
   `.weather-widget`に`box-sizing:border-box; max-width:100%; overflow:hidden`。
   幅超過分は`.weather-days`（overflow-x:auto）内で横スクロール
2. **個人持ち物のデフォルト変更**: 全種別共通で
   「ユニフォーム、シューズ、シンガード、ジャグ、お弁当」に（`checklist.js DEFAULT_ITEMS`）。
   ラベルを「🎒 個人持ち物」に変更し「種類を変えるとテンプレが入ります」文言を削除
3. **荷物（共用備品）機能を新設**:
   - 管理タブ「📦 荷物設定」: 名前入力→登録でマスタに追加。チップの✕で削除。
     保存先 `teams/fc-ojima/luggageSettings/main { items: [] }`（read:approved / write:manager）
   - イベント登録・編集フォームに「📦 荷物」ボタン → ピッカーモーダル。
     マスタの荷物が1行1点で並び、＋/−で数量増減（0未満不可・デフォルト0）。
     登録でフォームに戻り「テント×2、ベンチ×2」のサマリー表示
   - 保存先はイベントdocの `luggage: [{name, count}]`
   - 表示: カレンダーのイベント詳細モーダル・admin(マトリクス)イベントモーダル・
     配車概要（`Checklist.formatLuggage`）。配車概要には個人持ち物チップも表示
4. **添付ファイル強化**（共通モジュール `js/common/eventfiles.js` に集約）:
   - hubカレンダーのイベント詳細モーダルに添付一覧＋「ファイルを追加」を新設
     （保存先はStorageの `teams/{T}/events/{id}/` で配車管理画面と共通＝同期）
   - イベント登録・編集フォームからも添付可（保存時にアップロード）
   - 種類制限: PDF / Excel(xls,xlsx,csv) / Word(doc,docx) / 画像 / テキスト(txt,md)。
     1ファイル10MBまで。storage.rulesでもサイズ上限をサーバー側強制
   - **自動削除**: イベント終了後30日を過ぎた添付を、マネージャーがhubを開いた時に
     1日1回だけ掃除（Cloud Functionsを持たない静的構成のためのクライアント方式。
     従来「自動削除の指示があったはず」とのことだったが実装は存在しなかった）

### 検証
E2E `e2e-luggage-files.mjs` 19項目 全PASS（荷物設定の登録/削除・ピッカー増減と0下限・
イベント保存とモーダル/概要への表示・添付の型/サイズ検証・Storage実アップロード・
40日前イベントの添付自動削除・未来イベントの添付保持・天気の画面内表示）＋
天気表示ありのはみ出しチェックPASS。

### キャッシュバスト
`checklist.js`/`eventfiles.js`(新規)/`calendar.js`/`admin.js`/`main.js`/`overview.js`/
`db.js`/`firebase-config.js`/`common.css` → `?v=20260721a`

### 注意
`firestore.rules`（luggageSettings追加）と`storage.rules`（10MB上限）を変更している。
CIのデプロイでルールも反映される想定（firebase.jsonでrules指定済み）。

---

## 2026-07-21 実機フィードバック7件を修正（割り当て/送迎回数/当番グループ/参加率/マトリクス/重複チェック）

**ユーザー報告（実機スクショ2枚＋箇条書き）と対応**:

1. **座席割り当てに未回答の選手が出る** → 出欠「参加」回答済みの選手のみ表示に変更
   （`Assignment.getEventFilteredMembers`: playersを`presentNames`で絞る。未回答・欠席は非表示）
2. **「送迎の貢献度」→「送迎回数」に改名**。名前と回数の間のバーが常に空だったのを修正
   （`.driver-stat-bar-fill`が`span`のままinlineで幅が効いていなかった→`display:block`）
3. **当番グループのメンバー追加・削除ができない** → UIを刷新。
   作成時はメンバー0人、所属メンバーは「✕」ボタンで削除、「＋メンバーを追加」selectで追加
   （旧: 全候補チェックボックスのタップtrが実機で不安定だった）。他グループとの重複所属OK
4. **管理タブに「📊 選手の参加率」を追加**（`Admin.loadAttendanceRates`）。
   今シーズンの消化済みイベント（今日まで・ナイター除く）で、各選手が対象だった数と
   「参加」回答数から参加率を算出。6年→年少の学年降順・学年見出し付きで表示
5. **マトリクス見出しの参加/未回答/欠席が0だと灰色化されていた** → 0でも緑/黄/赤を維持
   （`z0`クラス付与を廃止）
6. **マトリクスの「未回答が多い選手」サマリーを廃止**（面積を取り、下部の6年生が
   見えなくなる原因だった）。関連の集計コードも削除
7. **イベント重複チェックの誤爆** → ハードエラーで登録拒否していたのを、
   重複内容を提示した上で`confirm`で登録続行できるように変更
   （「4〜6年イベントに3年生1人を追加後、同日に1〜3年イベントを作れない」詰みを解消）

### 検証
新規E2E `e2e-feedback7.mjs` 13項目 全PASS（①参加者のみ表示 ②送迎回数改名＋バー描画
③グループ空作成→select追加→✕削除 ④参加率2/2=100%・1/2=50% ⑤0の着色維持
⑥サマリー非表示 ⑦重複confirmで続行）。

### キャッシュバスト
`admin.js`/`calendar.js`/`assignment.js`/`common.css`/`checklist.js` → `?v=20260721`

---

## 2026-07-20 天気をウィジェット型に刷新（会場ピンポイント座標＋当日までの数日分予報）

**ユーザー要望**: 「太田市に座標を固定しないで。会場の場所の、イベント当日の天気を数日前に
知りたい。理想はYahoo天気ウィジェットのような数日分の予報がモーダル内に出ること。
無理ならリンクだけでもいい」→ 理想形を実装。

### 実装内容（`js/common/weather.js` 全面書き換え）
- **座標特定**: 国土地理院 住所検索API（`msearch.gsi.go.jp`、無料・キー不要・
  CORS許可を実測確認済み）で会場の登録住所→**町丁目レベルのピンポイント座標**。
  住所未登録の会場のみ従来のOpen-Meteo地名検索（市区町村精度）にフォールバック
- **表示**: `Weather.getDailyForecast()` が今日〜イベント当日の日別予報を取得し、
  `Weather.formatWidget()` がYahooウィジェット風の日別カード（日付・天気絵文字・
  最高/最低気温・降水確率）を生成。**当日のカードは黄枠＋「当日」バッジで強調**。
  期間が長い場合は当日側の直近6日分を表示。精度（📍会場周辺/市区町村）も表示
- **外部リンク**: どのケースでも「🔗 当日の会場の天気を調べる」
  （Google検索: `市区町村名 会場名 天気`）を必ず表示。予報範囲外
  （16日より先）・座標特定失敗時の逃げ道になる
- 表示箇所: hubイベント詳細モーダル・admin(マトリクス)イベントモーダル・
  carpool概要 の3箇所。旧 `getForecast`/`formatChip` はAPI互換で残置
- キャッシュ: 座標=localStorage永続(v4)・予報=sessionStorage当日中。
  API使用量は1人1日数リクエスト程度で70人規模でも無料枠に余裕

### 検証
E2E `e2e-weather-widget.mjs` 10項目 全PASS（GSIが最優先で呼ばれる・ピンポイント
座標が予報APIに渡る・当日ハイライト・リンク・座標特定失敗時のリンクのみ表示）。
スクリーンショットで見た目も確認済み。

### キャッシュバスト
`weather.js`/`calendar.js`/`admin.js`/`overview.js`/`common.css` → `?v=20260720b`
