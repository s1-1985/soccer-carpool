...
①②③⑤⑥はすべて完了・本番反映済み。残るは**④当番表のみ**。

1. まずこのファイルを読む
2. ユーザーに④当番表の詳細仕様をヒアリングする（本ファイル「4. 当番表機能」の
   「要件（要ユーザーヒアリング）」節にある4つの質問：当番の種類・人数、
   ローテーションルール、割り当て方法、通知要否）
3. ヒアリング後、同セクションの実装方針（Firestoreルール変更が必須である点に注意）に従って実装
